...
*   **Generated Source Guard**: Common generated artifacts are removed before parse/chunk work. The scanner skips generated directories, generated Dart suffixes, and Swift protobuf files (`*.pb.swift`), then checks the first bytes of supported code files for generated-source headers such as protoc output (`Generated by the protocol buffer compiler. DO NOT EDIT!`). Legacy, staged, and incremental indexing all keep the same guard so generated sources cannot be reintroduced by a watcher update.
*   **Post-Publish Embedding Backfill**: Full indexing commits chunks, symbols, relations, BM25, and serving generation before vector embedding enqueue. This keeps code search structurally available even when a large local embedding model is slow; vector rows are filled by the background embedding worker afterward. Startup recovery scans all persisted `embedding_pending` projects and re-queues unembedded chunks/symbols independently of the optional `--project-path` lifecycle manager. Vector/semantic serving generations are published only after HNSW rebuild succeeds.
